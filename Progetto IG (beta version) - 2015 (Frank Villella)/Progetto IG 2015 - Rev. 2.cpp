...
void Spawn()
{
	...
	Mix_PlayMusic(music, -1);
	sprofondato = false;
	glutTimerFunc(4000, stopMusic, 2);
	time_remaining = 0;
}
...
void initText(string mystring, float x, float y)
{
	...
	sprintf(buf, "Health: %d", health_status);
...
void keyboard(unsigned char key, int w, int h)
{
	...
	if (key == 'w')
	{
		if ((x + lx * fraction < 93 && x + lx * fraction >= -3) && (z + lz * fraction >= -3 && z + lz * fraction <= 93) && parete_1())
		{
			x += lx * fraction;
			z += lz * fraction;		
		}
...
void display()
{
	...
	initText("Health Status:", -0.9, 0.8);
